...
def dimdb_bildirimi_gonder(barkod, agirlik, materyal_turu, uzunluk, genislik, kabul_edildi, sebep_kodu, sebep_mesaji):
    ...
    try:
        ...
        # Basit thread-based async wrapper
        def send_in_background():
            try:
                ...
                try:
                    ...
                    print(f"❌ [DİM-DB] Send hatası: {e}")
                finally:
                    loop.close()
            except Exception as e:
                print(f"❌ [DİM-DB] Background thread hatası: {e}")
        
        # Background thread'de çalıştır - non-blocking
...
def dogrulama(barkod, agirlik, materyal_turu, uzunluk, genislik):
    ...
    if min_genislik-20 <= genislik <= max_genislik+20:
    ...
    if min_uzunluk-20 <= uzunluk <= max_uzunluk+20 :
...
def yonlendirici_hareket():
    ...
    # İade aktifse yönlendirme yapma
    if giris_iade_lojik:
        print(f"🚫 [YÖNLENDİRME] İade aktif, yönlendirme iptal edildi")
        return
...
def mesaj_isle(mesaj):
    ...
    if mesaj == "yso":
        ...
        print(f"🔍 [DEBUG] YSO geldi - Kuyruk durumu: {len(kabul_edilen_urunler)} ürün")
        yonlendirici_hareket()

t1 = threading.Thread(target=goruntu_isleme_tetikle(), daemon=True)
t1.start()
